...
HANDLE GetFileContent(const char* filePath){
    ...
    const HANDLE hFile = CreateFileA(filePath, GENERIC_READ, 0, nullptr, OPEN_EXISTING, 0, nullptr);
    ...
    const DWORD_PTR dFileSize = GetFileSizeEx(hFile, nullptr);
...
int main(const int argc, char* argv[]){
    ...
    char* filePath;//prepare pointer to save file path

    if(argc == 0){//check if there's any passed arguments
        printf("Please pass some arguments\n");
        return -1;
    }
    filePath = argv[1];//take in passed file length
    
    const HANDLE hFileContent = GetFileContent(filePath);
